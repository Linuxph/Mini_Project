      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {
          Array.from({ length: 8 }).map((_, index) => (
            ...
            <div key={index} className="w-full bg-white rounded-2xl shadow-md overflow-hidden transform transition-transform hover:scale-105 cursor-pointer">
              <img src={imageUrl} alt="Food Item" className="w-full h-40 object-cover" />
              <div className="p-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button  onclick={handleClick} className="bg-orange-500 text-white text-sm px-3 py-1 rounded-full hover:bg-orange-600 transition">Add</button>
                </div>
              </div>
            </div>
          ))
